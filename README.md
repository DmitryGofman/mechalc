...
## Branding
Logo candidates live in `public/brand/<design>/` — four base designs
(`blueprint`, `beam`, `hexm`, `gauge`) plus four stress-beam variants painted
by signed bending stress (`beamsplit`, `beamramp`, `beamfringe`, `beamfield`;
generated by `scripts/gen-beam-variants.mjs`) — previewed side by side —
home-screen, tab and header mockups — at `public/designs/brand/index.html`. The active design is named by
